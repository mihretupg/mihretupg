  ...
  return (
    <header className="fixed left-0 right-0 top-0 z-50 border-b border-white/20 bg-navy-950/92 backdrop-blur">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8">
        ...
        <button
          type="button"
          className="grid h-10 w-10 place-items-center rounded-lg border border-white/20 text-white transition hover:border-accent-400 hover:bg-white/10 lg:hidden"
...
function NavLink({ item, mobile = false, onClick }) {
  return (
    <a
      className={
        mobile
          ? "rounded-lg px-3 py-3 text-sm font-medium text-resume-line transition hover:bg-white/10 hover:text-white"
          : "rounded-lg px-3 py-2 text-sm font-medium text-resume-line transition hover:bg-white/10 hover:text-white"
      }
